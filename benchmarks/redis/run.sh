...
do
  ...
  pushd .
  cd $YCSB_PATH
  $YCSB_PATH/bin/ycsb load redis -s -P $YCSB_PATH/workloads/workload$i -p "redis-host=127.0.0.1" -p "redis.port=6379"
  perf record -o /tmp/perf.data -g -p $server_pid -- $YCSB_PATH/bin/ycsb run redis -s -P $YCSB_PATH/workloads/workload$i -p "redis-host=127.0.0.1" -p "redis.port=6379"
  popd
