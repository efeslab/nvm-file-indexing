...
static int read_radix_node(radixtree_meta_t *radix, radix_node_t *node) {
    int err = read_page(radix, node->rn_page, node->rn_dev_contents);
    if (err) return err;
    ...
    for (size_t i = 0; i < radix->ents_per_node; ++i) {
        ...
        if (!n) {
            err = create_radix_node(radix, entry, depth, n);
            if (err) return err;
        } else {
            n->rn_depth = depth;
            n->rn_page  = entry;
                             ...
                             int depth, radix_node_t *node) 
{
    ...
    if (!node->rn_dev_contents || !node->rn_cache_tree) return -ENOMEM;
                                ...
                                radix_node_t *node, size_t idx) 
{
    ...
    if (!child) {
        ...
        if (!entry) {
            ...
            node->rn_cache_tree[idx].rn_page = entry;
            node->rn_cache_tree[idx].rn_cache_state = -1;
            int ret = write_page(radix, node->rn_page, node->rn_dev_contents);
            if (ret) return NULL;
        }
        int err = create_radix_node(radix, entry, node->rn_depth + 1, child);
        if (err) return NULL;
    }
